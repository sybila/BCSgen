...
class Rule:
    ...
    def checkSolutionAndLhs(self, solution):
        return False not in map((lambda (a, b): a.isCompatibleWith(b)), zip(solution, self.left_hand_side))
    ...
    """
    Call replace function on all UNIQUE (!!!) solutions generated by matching
    :param solution_original: given solution (array)
    ...
    def dissociateComplex(self, solution_original):
        ...
        for agent in self.right_hand_side:
            if isinstance(agent, Complex_Agent):
                result.append(Complex_Agent(sorted([solution.pop(0) for _ in range(len(agent.getFullComposition()))]), agent.getCompartment()))
            else:
