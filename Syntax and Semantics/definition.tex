...
Let $\beta$ be an agent, $\mathtt{R} \in \mathcal{E}_\mathcal{R}$ be a rule of form $\lambda \Rightarrow \rho$ where $\lambda$ and $\rho$ are agent enumerations, $\mathtt{State} \subseteq \mathcal{U}$ be a state, and $\mathtt{S} \subseteq \mathtt{State}$ be a solution. 
...
\begin{tabular}{ c | | c | l }
    ...
    % dissotiation
     ...
     & 
    $ \mathtt{X}_\mathtt{S}[\Gamma_\rho] = \Gamma_\mathtt{R} $ & 
    $ \mathtt{X}_\mathtt{S} = \gamma_{f_\mathtt{S}}::c $ ~~~~~ $ \gamma_{f_\mathtt{S}} = \beta_{\mathtt{S}_1}.\beta_{\mathtt{S}_2}.~...~.\beta_{\mathtt{S}_n} $\\

     &  & $ \Gamma_\mathtt{R} = \beta_{\mathtt{R}_1}::c + \beta_{\mathtt{R}_2}::c + ~...~+ \beta_{\mathtt{R}_n}::c : \beta_{\mathtt{R}_i} = \beta_{\mathtt{S}_i}$ \\
	...
	$\frac{\beta_{\mathtt{S}_1}~\lhd~\beta_{\lambda_1},~\beta_{\mathtt{S}_2}~\lhd~\beta_{\lambda_2},~...,~\beta_{\mathtt{S}_n}~\lhd~\beta_{\lambda_n}}{\beta_{\mathtt{S}_1} + \beta_{\mathtt{S}_2} + ~...~+ \beta_{\mathtt{S}_n} ~\models ~\beta_{\lambda_1} + \beta_{\lambda_2} + ~...~+ \beta_{\lambda_n}}$ & 
    $ \mathtt{S}_\Upsilon[\mathtt{X}_\rho] = \mathtt{X}_\mathtt{R} $ & 
    $ \mathtt{S}_\Upsilon = \beta_{\mathtt{S}_1}::c + \beta_{\mathtt{S}_2}::c + ~...~+ \beta_{\mathtt{S}_n}::c $\\

     &  & $ \mathtt{X}_\mathtt{R} = \beta_{\mathtt{R}_1}.\beta_{\mathtt{R}_2}.~...~.\beta_{\mathtt{R}_n}::c : \beta_{\mathtt{R}_i} = \beta_{\mathtt{S}_i} $ \\
	...
	$\frac{\Gamma_\mathtt{S}~\lhd~\Gamma_{\lambda}}{\Gamma_\mathtt{S}~\models~\Gamma_{\lambda}}$ & 
    $ \Gamma_\mathtt{S}[\emptyset] = \emptyset $ & \\
    ...
    $ \emptyset[\Gamma_\rho] = \Gamma_\mathtt{R} $ & 
    $\Gamma_\mathtt{R} = \Gamma_\rho $ \\
	...
	$ \frac{\mathcal{E}_\mathtt{S}~\lhd~\mathcal{E}_\rho}{\mathcal{E}_\mathtt{S}~\models~\mathcal{E}_\rho} $ & 
    $ \mathcal{E}_\mathtt{S}[\mathcal{E}_\rho] = \mathcal{E}_\mathtt{R} $ & 
    $ \mathcal{E}_\mathtt{S} = \beta_\mathtt{S}::c_\mathtt{S} $ ~~~~~ $ c_\mathtt{S} \neq c_\rho $ \\

    &  & $ \mathcal{E}_\rho = \beta_\rho::c_\rho $ ~~~~~ $ c_\rho = c_\mathtt{R} $\\

    &  & $ \mathcal{E}_\mathtt{R} = \beta_\mathtt{R}::c_\mathtt{R} $ ~~~~~ $ \beta_\mathtt{R} = \beta_\rho = \beta_\mathtt{S} $\\
...
\begin{defn}
$\emph{Rule application}$ is mapping $\mathtt{State}_{\blacksquare} \times \mathtt{R} \rightarrow \{\mathtt{State}\}$ such that for the $\mathtt{State}_{\blacksquare}$ there exists solution $\mathtt{S}_{\blacksquare} \subseteq \mathtt{State}_{\blacksquare}$: $\forall ~\mathtt{S}_{\Upsilon_{\blacksquare}} \in \Upsilon_{\mathtt{S}_{\blacksquare}}$ we get a new solution $\mathtt{S}$ by application of the rule $\mathtt{R}$ (defined above) such that $\mathtt{S} \subseteq \mathtt{State}$.
\end{defn}

\begin{defn}
$\emph{Rule-based model}$ $\mathcal{M}$ is a pair $ (\mathtt{State},~\mathcal{R}) $ such that $\mathcal{R}$ is set of rules $\mathtt{R}$. The model yields $\emph{transition system}$ between states by applying $\emph{Rule application}$.
\end{defn}
