...
where $I$ is set of indices of $\mathtt{X}_s$.
...
\begin{itemize}
	\item $ f(\alpha) = \gamma_f(\alpha)$ when $\mathtt{X} \multimap \alpha $
	\item $ f(\alpha) = [\alpha]$ when $\mathtt{T} \multimap \alpha$
	\item $ f(\alpha) = [\alpha]$ when $\mathtt{a} \multimap \alpha$
\end{itemize}
...
\begin{itemize}
\item $ \pmb{\mathtt{X}_s[\Gamma_r] = \Gamma_s}:$

$\iota(*list*)$ = [ ($ i, \alpha_i $) $\forall \alpha \in \mathtt{X}_s$ ]

$map(\iota(*list*)$, I = \{n, n+1, ..., n+j\}) = [ $\alpha_i~|~\forall i \in I $ ] 

cut($\mathtt{X}_r$) = [  ]

to solve: we need iota for list and also list of lists !!!
