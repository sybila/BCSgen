...
With assumption all rules of form $\Gamma_l~\Rightarrow~\Gamma_r$ are well-formed, we can define \textbf{Matching} as a relation $\models \subseteq \Gamma_s~\times~\Gamma_l $.
...
\begin{tabular}{ l l }
	...
	$ \mathtt{T}_s \models \mathtt{T}_l $ & $ \mathtt{T}_s[\mathtt{T}_r] = \mathtt{T}_s:  \forall a\{t\} \in \mathtt{T}_r: a_s\{p\}[a_r\{t\}] $\\
	$ \mathtt{X}_s \models \mathtt{X}_l $ & $ \mathtt{X}_s[\mathtt{X}_r] = \mathtt{X}_s: \forall \alpha_r \in \mathtt{X}_r: \alpha_s[\alpha_r] $
\end{tabular}
\end{center}

where $ a_s \in \Gamma_s, a_l \in \Gamma_l, a_r \in \Gamma_r, a_s = a_l = a_r, \alpha_s \in \mathtt{X}_s $
