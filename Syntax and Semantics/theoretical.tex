...
\begin{itemize}
\item $ \pmb{\mathtt{a}_s\{p\}::c_s[\mathtt{a}_r\{t\}::c_r] = \mathtt{a}_s\{t\}::c_r} $

where $ \mathtt{a}_s = \mathtt{a}_l = \mathtt{a}_r $.

\item $ \pmb{\mathtt{T}_s::c_s[\mathtt{T}_r::c_r] = \mathtt{T}_s::c_r}: \newline \forall \mathtt{a}_r\{t\} \in \gamma_p^{r} : \mathtt{a}_s\{p\} \in \gamma_p^{s} : \mathtt{a}_s\{p\}::c_s[\mathtt{a}_r\{t\}::c_r] $

where $\mathtt{T}_s = \tau_s(\gamma_p^{s})$, $\mathtt{T}_l = \tau_l(\gamma_p^{l})$, $\mathtt{T}_r = \tau_r(\gamma_p^{r})$ $\newline$ and $\tau_s = \tau_l = \tau_r$ and $\mathtt{a}_s = \mathtt{a}_r$.

\item $ \pmb{\mathtt{X}_s::c_s[\mathtt{X}_r::c_r] = \mathtt{X}_s::c_r}: \newline \forall \alpha_1, \alpha_2, ..., \alpha_n \in \mathtt{X}_s $ and $\forall \beta_1, \beta_2, ..., \beta_n \in \mathtt{X}_r : \alpha_i::c_s[\beta_i::c_r]$
...
\begin{itemize}
\item $\pmb{\Gamma_s[\Gamma_r] = \gamma_f} : \newline ([\alpha~|~\forall \alpha \in \mathtt{X}]~|~ \forall \mathtt{X} \in \Gamma_s) + [ \mathtt{T}~|~\forall\mathtt{T} \in \Gamma_s] + [ \mathtt{a}~|~\forall \mathtt{a} \in \Gamma_s ] $
\end{itemize}

\item \textit{complex dissociation}:

\begin{itemize}
\item $ \pmb{\gamma_f(\Gamma_s)[\Gamma_r] = \Gamma_s}:$

$result = [~]$

$\forall \alpha::c \in \Gamma_r:$

$if~isinstance(\alpha, \mathtt{X}):$

$\hspace{1cm} result \leftarrow \mathtt{X}([\Gamma_s.pop(0)~for~i~in~range(len(\gamma_f(\alpha))],~c)$

$ else: $

$\hspace{1cm} result \leftarrow \Gamma_s.pop(0)$
\end{itemize}
