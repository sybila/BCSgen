...
\begin{mydef}
With assumption all rules of form $\Gamma_l\Rightarrow\Gamma_r$ are well-formed, we define \textbf{Matching} as relation $\models \subseteq \Gamma_s~\bigtimes~\Gamma_l $ via \textit{compatibility} operator:\\ $\forall (\alpha_s,~\alpha_l)~\in~\models~: \alpha_s \in \Gamma_s $ and $ \alpha_l \in \Gamma_l$ and $\alpha_s \lhd \alpha_l$. 

Moreover, $\forall (\alpha, \alpha'), (\beta, \beta') \in \models: \alpha~\neq~\beta~\Leftrightarrow~\alpha'~\neq~\beta' $.
\end{mydef}
...
Let $\gamma^{r-l}$ is the difference between $\gamma^{l}$ and $\gamma^{r}$ compositions. Then $\gamma^{s^-}$ is part of $\gamma^{s}$ composition such that there exist relation $\rho \subseteq \gamma^{r-l} \bigtimes \gamma^{s^-}:$ $\forall~\alpha_1~\in~\gamma^{s^-}~\exists~\alpha_2~\in~\gamma^{r-l}:~(\alpha_1,~\alpha_2)~\in~\rho~\wedge~\alpha_1~\lhd~\alpha_2$. Moreover, $\forall (\alpha, \alpha'), (\beta, \beta') \in \rho: \alpha~\neq~\beta~\Leftrightarrow~\alpha'~\neq~\beta' $.

\begin{itemize}
\item	\begin{tabular}{ l l }
				$ \mathtt{a}_s\{p\} \models \mathtt{a}_l $ & \hspace*{0.6 cm} $ \mathtt{a}_s\{p\}[\mathtt{a}_r\{t\}] = \mathtt{a}_s\{t\} $
		\end{tabular}
	  
where $ \mathtt{a}_s = \mathtt{a}_l = \mathtt{a}_r $.
\item   \begin{tabular}{ l l }
				$ \mathtt{T}_s \models \mathtt{T}_l $ & \hspace*{1 cm} $ \mathtt{T}_s[\mathtt{T}_r] = \mathtt{T}_s: \forall (\mathtt{a}_r\{t\},~\mathtt{a}_s\{p\}) \in \rho: \mathtt{a}_s\{p\}[\mathtt{a}_r\{t\}] $
		\end{tabular}
		
where $\mathtt{T}_s = \tau_s(\gamma^{s})$, $\mathtt{T}_l = \tau_l(\gamma^{l})$, $\mathtt{T}_r = \tau_r(\gamma^{r})$ and $\tau_s = \tau_l = \tau_r$.

\item   \begin{tabular}{ l l }
			$ \mathtt{X}_s \models \mathtt{X}_l $ & \hspace*{1 cm} $ \mathtt{X}_s[\mathtt{X}_r] = \mathtt{X}_s: \forall (\alpha_r,~\alpha_s) \in \rho: \alpha_s[\alpha_r] $
		\end{tabular}
...
\begin{center}
\begin{tabular}{ l l }
	$ \Gamma_s \models \Gamma_l $ & \hspace*{1 cm} $\Gamma_s[\Gamma_r] = \gamma_f : \{ x~|~x \in \gamma_f~\forall\mathtt{X} \in \Gamma_s \vee x = \mathtt{T}~\forall\mathtt{T} \in \Gamma_s \vee x = \mathtt{a}~\forall \mathtt{a} \in \Gamma_s \}$
\end{tabular}
\end{center}

\item \textit{complex dissociation}:

challenging part, have to make sure that right hand side matches new solution
\begin{center}
\begin{tabular}{ l l }
