...
\hspace*{-1cm}\begin{tabular}{ l l l l }
 ...
 partial composition & $\gamma_p ::= \mathtt{A}_1, \mathtt{A}_2, ..., \mathtt{A}_n$ & atomic agent & $\mathtt{A}_i ::= n \in \mathcal{E}_\mathcal{A}$\\
 & & compartment & $\mathtt{c} ::= n \in \mathcal{N}_{c}$\\
\end{tabular}
}
\end{center}   
...
\begin{theorem}
Since structure agents are constructed from finite sets $\mathcal{N}_{T},~\mathcal{N}_{c}$, and $\mathcal{A}$, we can define universe of structure agents

 $\mathcal{T} = \{ \mathtt{T}~|~\mathtt{T} = (\tau, \gamma_p, \mathtt{c}), \tau~\in~\mathcal{N}_{T}, \gamma_p~\subseteq~\mathcal{A},  \mathtt{c}~\in~\mathcal{N}_{c} \}$.

\noindent Similarly, we can define universe of structure agent expressions
...
\hspace*{-1 cm}\begin{tabular}{ l l l l }
 ...
 full composition & $\gamma_f ::= \beta_1.\beta_2.~...~.\beta_m$ & $\beta_i ::= \mathtt{T}~\choice~\mathtt{A} $\\
 structure agent & $\mathtt{T} ::= n \in \mathcal{E}_\mathcal{T}$ & atomic agent & $\mathtt{A} ::= n \in \mathcal{E}_\mathcal{A}$
\end{tabular}
}
\end{center}
...
\begin{theorem}
Since complex agents are constructed from finite sets $\mathcal{N}_{c}$, $\mathcal{A}$, and $\mathcal{T}$, we can define universe of complex agents 

$\mathcal{X} = \{ \mathtt{X}~|~\mathtt{X} = (\gamma_f, \mathtt{c}), \gamma_f \subseteq \mathcal{A} \cup \mathcal{T},  \mathtt{c}~\in~\mathcal{N}_{c} \}$.

\noindent Similarly, we can define universe of complex agent expressions
...
~
\begin{itemize}
\item Agent enumeration $\Gamma$ is an expression of form $\varphi_1 + \varphi_2 + ... + \varphi_n$. From now, when referencing it, it might be treated as a tuple $(\varphi_1, \varphi_2, ..., \varphi_n)$.
\end{itemize}
\end{notation}
...
\begin{defn}
$ $ \\
\textbf{Matching} is a function $\models ~: \Upsilon_\mathtt{S}~\bigtimes~\lambda~\rightarrow~\Upsilon_\mathtt{S}^{\models}$ defined particularly:
...
\begin{defn}
$ $ \\
\textbf{Replacement} is a function $\Upsilon_\mathtt{S}^{\models}~\times~\rho~\rightarrow~\Upsilon_\mathtt{R}$, where $\Upsilon_\mathtt{R}$ is set of agent enumerations created according to six replacement categories:
