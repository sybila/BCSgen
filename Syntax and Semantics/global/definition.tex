...
We allow rules to be bi-directional -- it is just a shortcut for two rules and it can be converted to basic rule form. A rule $\mathtt{R} = \lambda ~\Leftrightarrow~ \rho~\nu$ might be written as two rules $\mathtt{R}_1 = \lambda ~\Rightarrow~ \rho~\nu$ and $\mathtt{R}_2 = \rho ~\Rightarrow~ \lambda~\nu$. Similarly for all rule forms.

\subsection{Stoichiometry}

In agent enumeration $\Gamma = (\varphi_1, \varphi_2, ..., \varphi_n)$, we enrich $\varphi_i$ context to pair $(\zeta, \varphi_i)$ where $\zeta = 1$. However, some rule agents in $\Gamma$ might be equivalent. For this reason, there are equivalence groups $\mathcal{G}_\equiv$ of \textit{k} rule agents $\varphi_i, ..., \varphi_j$. Each of there groups $\mathcal{G}_\equiv$ might be written as one pair $(\zeta, \varphi_i)$ where $\zeta = k$ for some $\varphi_m \in \mathcal{G}_\equiv$.
...
In a rule, one rule agent might be referenced using variable $\upsilon$ as a set of agents it can be replaced with. Such rule agent is referenced as $?X$. The appropriate set of agents is referenced via following grammar: 
