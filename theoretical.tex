...
\vspace{1cm}
\textbf{TBA} well-defined rule expression item might be incorrect !
...
\item the number of interacting molecules must be \textbf{fixed} ($\#(LHS)~=~\#(RHS)$). Exceptions:
	\begin{enumerate}
		...
		\item (de)complexation - number of new agents is different BUT number of all structure agents must be fixed
	\end{enumerate}
...
\textbf{TBA} complex can be formed from atomic and also structure agents, check if it has any influence!
...
\textbf{Note:} Meaning of $a \in \mathtt{A}$ changes considering different types of agents, but it always expresses fact that the agent $a$ is present in agent $\mathtt{A}$.

With assumption all rules ($\{\mathtt{A}_l\} \Rightarrow \{\mathtt{A}_r\}$) are well-formed, we can define \textbf{Replacement}, what is a function $\{\mathtt{A}_s\} \times \{\mathtt{A}_l\} \rightarrow \{\mathtt{A}_r\}$. Before replacement can be applied, matching must be satisfied. \textbf{Matching} is defined as a relation $\models \mathtt{A}_s \times \mathtt{A}_l $ according to reaction type of the rule:

\begin{enumerate}
\item state change: $\mathtt{A}_l \Rightarrow \mathtt{A}_r$

\begin{tabular}{ l l }
\textbf{if} & $\mathtt{A}_s \models \mathtt{A}_l$ iff $\mathtt{A}_s \lhd \mathtt{A}_l$\\
\textbf{then} & $\forall a\{t\} \in \mathtt{A}_r : a\{t\} \notin \mathtt{A}_l$ and $a_s \in \mathtt{A}_s : a\{t\} \neq a_s$: $ a_s [a\{t\}] = a_s\{t\}$\\
\textbf{solution} & agent $\mathtt{A}_s$ with changed state(s)
\end{tabular}

\item complex formation: $\{\mathtt{A}_l\} \Rightarrow \mathtt{A}_r$

\begin{tabular}{ l l }
\textbf{if} & $\{\mathtt{A}_s\} \models \{\mathtt{A}_l\}$ iff $\forall \mathtt{A}_s \in \{\mathtt{A}_s\}: \exists \mathtt{A}_l \in \{\mathtt{A}_l\}: \mathtt{A}_s \lhd \mathtt{A}_l$\\
\textbf{then} & $\forall \mathtt{A}_s \in \{\mathtt{A}_s\}: \mathtt{A}_s \rightarrow \gamma_f$ ($\mathtt{X} = \mathtt{A}_r = \gamma_f$)\\
\textbf{solution} & agent $\mathtt{A}_r = \gamma_f$
\end{tabular}

\item complex dissociation: $\mathtt{A}_l \Rightarrow \{\mathtt{A}_r\}$

\begin{tabular}{ l l }
\textbf{if} & $\mathtt{A}_s \models \mathtt{A}_l$ iff $\mathtt{A}_s \lhd \mathtt{A}_l$\\
\textbf{then} & $\forall \mathtt{A}_r \in \{\mathtt{A}_r\}: \mathtt{A} \in \mathtt{A}_s: \mathtt{A} = \mathtt{A}_r: \mathtt{A} \rightarrow \{\mathtt{A}\} $\\
\textbf{solution} & list of agents $\{\mathtt{A}\}$ 
\end{tabular}

\item degradation $\{\mathtt{A}_l\} \Rightarrow$

\begin{tabular}{ l l }
\textbf{if} & $\{\mathtt{A}_s\} \models \{\mathtt{A}_l\}$ iff $\forall \mathtt{A}_s \in \{\mathtt{A}_s\}: \exists \mathtt{A}_l \in \{\mathtt{A}_l\}: \mathtt{A}_s \lhd \mathtt{A}_l$\\
\textbf{then} & $\forall \mathtt{A}_s \in \{\mathtt{A}_s\}: \mathtt{A}_s \rightarrow \emptyset $\\
\textbf{solution} & $ \emptyset $
\end{tabular}

\item translation $\Rightarrow \{\mathtt{A}_r\}$

\begin{tabular}{ l l }
\textbf{if} & True\\
\textbf{then} & $\forall \mathtt{A}_r \in \{\mathtt{A}_r\}: \mathtt{A}_r \rightarrow \{\mathtt{A}\}$\\
\textbf{solution} & list of agents $\{\mathtt{A}\}$ 
\end{tabular}
\end{enumerate}
